...
namespace
{
	...
	struct Device
	{
		...
		void CaptureScreenTransform()
		{
			ScreenTransform.RenderTarget = SDL_GetRenderTarget(Renderer);
			SDL_RenderGetLogicalSize(Renderer, &ScreenTransform.LogicalWidth, &ScreenTransform.LogicalHeight);
			SDL_RenderGetViewport(Renderer, &ScreenTransform.Viewport);
			SDL_RenderGetScale(Renderer, &ScreenTransform.ScaleX, &ScreenTransform.ScaleY);
		}

		void RestoreScreenTransform()
		{
			SDL_SetRenderTarget(Renderer, ScreenTransform.RenderTarget);
			if (ScreenTransform.LogicalWidth > 0 && ScreenTransform.LogicalHeight > 0)
			{
				SDL_RenderSetLogicalSize(Renderer, ScreenTransform.LogicalWidth, ScreenTransform.LogicalHeight);
			}
			else
			{
				SDL_RenderSetViewport(Renderer, &ScreenTransform.Viewport);
				SDL_RenderSetScale(Renderer, ScreenTransform.ScaleX, ScreenTransform.ScaleY);
			}
		}
...
namespace ImGuiSDL
{
	...
	{
		if (!window || !renderer || logicalWidth <= 0 || logicalHeight <= 0)
		{
			return;
		}
		...
		const float outputMouseX = io.MousePos.x * static_cast<float>(outputWidth) / static_cast<float>(windowWidth);
		const float outputMouseY = io.MousePos.y * static_cast<float>(outputHeight) / static_cast<float>(windowHeight);
		const float scaleX = static_cast<float>(outputWidth) / static_cast<float>(logicalWidth);
		const float scaleY = static_cast<float>(outputHeight) / static_cast<float>(logicalHeight);
		...
		const float viewportWidth = static_cast<float>(logicalWidth) * scale;
		const float viewportHeight = static_cast<float>(logicalHeight) * scale;
		const float viewportX = (static_cast<float>(outputWidth) - viewportWidth) * 0.5f;
		const float viewportY = (static_cast<float>(outputHeight) - viewportHeight) * 0.5f;

		if (outputMouseX < viewportX || outputMouseY < viewportY ||
			outputMouseX > viewportX + viewportWidth || outputMouseY > viewportY + viewportHeight)
		{
		...
		io.MousePos = ImVec2(
			(outputMouseX - viewportX) / scale,
			(outputMouseY - viewportY) / scale);
	}
	...
	{
		if (!CurrentDevice || !drawData)
		{
			return;
		}

		CurrentDevice->CaptureScreenTransform();
		...
		CurrentDevice->RestoreScreenTransform();
