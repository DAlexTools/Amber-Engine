...
SceneObject* SceneDocument::FindObject(uint32 id)
{
...
const SceneObject* SceneDocument::FindObject(uint32 id) const
{
...
bool SceneDocument::IsObjectRemovable(uint32 id) const
{
...
AE::Scene::Document SceneDocument::ToRuntimeDocument() const
{
	...
	for (const SceneObject& object : objects)
	{
		...
		sceneObject.className = object.className.empty() ? (object.kind == SceneObjectKind::AssetInstance ? "SpriteObject" : object.kind == SceneObjectKind::Box  ? "BoxObject"
...
bool SceneDocument::LoadFromFile(const std::filesystem::path& path, std::string* error)
{
	...
	for (const AE::Scene::ObjectData& sceneObject : document.objects)
	{
		...
		object.className = sceneObject.className.empty() ? (kind == SceneObjectKind::AssetInstance ? "SpriteObject" : kind == SceneObjectKind::Box	? "BoxObject"
