...
struct PhysicsBodyDefinition
{
	...
	uint32 collisionCategory = EnginePhysicsCollision::Player;
	uint32 collisionMask = EnginePhysicsCollision::All;
	bool isSensor = false;
};
...
public:
	...
	{
		std::transform(shapeName.begin(), shapeName.end(), shapeName.begin(), [](unsigned char character)
