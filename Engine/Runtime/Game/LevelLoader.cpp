...
uint32 ReadCollisionLayerValue(const sol::object& value, uint32 defaultValue)
{
	...
	if (value.is<int>())
	{
		return static_cast<uint32>(value.as<int>());
	}

	if (value.is<double>())
	{
		return static_cast<uint32>(value.as<double>());
	}

	return defaultValue;
}

uint32 ReadCollisionLayer(sol::table table, const char* key, const char* fallbackKey, uint32 defaultValue)
{
...
uint32 DefaultPhysicsCollisionCategory(const std::string& entityTag, const std::string& entityGroup)
{
	if (entityTag == "player")
	{
...
uint32 DefaultPhysicsCollisionMask(const std::string& entityTag, const std::string& entityGroup)
{
	if (entityTag == "player")
	{
	...
	if (tilePhysicsBody != sol::nullopt && ReadBool(tilePhysicsBody.value(), "enabled", true))
	{
		...
		else
		{
			...
			else
			{
				...
				const uint32 collisionCategory = ReadCollisionLayer(
					tilePhysicsConfig,
					"collision_category",
					"category",
					EnginePhysicsCollision::Obstacle);
				const uint32 collisionMask = ReadCollisionLayer(
					tilePhysicsConfig,
					"collision_mask",
					"mask",
					EnginePhysicsCollision::All);
