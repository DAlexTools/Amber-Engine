...
{
    const AE::Physics::FVector2D a(3.0f, 4.0f);
    const AE::Physics::FVector2D b(-1.0f, 2.0f);

    const AE::Physics::FVector2D sum = a + b;
    EXPECT_FLOAT_EQ(sum.X, 2.0f);
    EXPECT_FLOAT_EQ(sum.Y, 6.0f);

    const AE::Physics::FVector2D diff = a - b;
    EXPECT_FLOAT_EQ(diff.X, 4.0f);
    EXPECT_FLOAT_EQ(diff.Y, 2.0f);

    const AE::Physics::FVector2D scaled = a * 2.0f;
...
{
    const AE::Physics::FVector2D vector(3.0f, 4.0f);
    const AE::Physics::FVector2D other(2.0f, -1.0f);
...
TEST(FVector2DTests, NormalizesWithoutMutatingOriginalWhenRequestingUnitVector)
{
    ...
    const AE::Physics::FVector2D unit = vector.UnitVector();
...
{
    const AE::Physics::FVector2D right(1.0f, 0.0f);

    const AE::Physics::FVector2D up = right.Rotate(Pi * 0.5f);
