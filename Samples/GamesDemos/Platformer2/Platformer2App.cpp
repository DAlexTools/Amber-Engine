...
bool Platformer2App::LoadMap()
{
	...
	while (std::getline(input, line))
	{
		...
		if (token == "tile")
		{
			...
			if (kindToken == "Solid")
				kind = TileKind::Solid;
			else if (kindToken == "Ladder")
				kind = TileKind::Ladder;
			else if (kindToken == "Spike")
				kind = TileKind::Spike;
			else if (kindToken == "Coin")
				kind = TileKind::Coin;
			else if (kindToken == "Goal")
				kind = TileKind::Goal;
			else if (kindToken == "Decor")
				kind = TileKind::Decor;
...
bool Platformer2App::SaveMap() const
{
	...
	for (int y = 0; y < LevelRows; ++y)
	{
		...
		{
			const TileCell& cell = level[static_cast<SizeT>(y)][static_cast<SizeT>(x)];
	...
	{
		output << "enemy " << enemy.spawnPosition.x << ' ' << enemy.spawnPosition.y << ' ' << enemy.leftBound << ' ' << enemy.rightBound << ' ' << enemy.speed << ' ' << std::max(1, enemy.health) << '\n';
	}

	for (const Lift& lift : lifts)
	{
		output << "lift " << lift.basePosition.x << ' ' << lift.basePosition.y << ' ' << lift.axis.x << ' ' << lift.axis.y << ' ' << lift.width << ' ' << lift.height << ' ' << lift.amplitude << ' ' << lift.speed << ' ' << lift.phase << '\n';
	}
...
void Platformer2App::SetTile(int x, int y, TileKind kind, int visual)
{
	...
	level[static_cast<SizeT>(y)][static_cast<SizeT>(x)] = TileCell{kind, visual};
}
...
void Platformer2App::UpdatePickupsAndHazards()
{
	...
	for (int y = top; y <= bottom; ++y)
	{
		...
		{
			TileCell& cell = level[static_cast<SizeT>(y)][static_cast<SizeT>(x)];
...
bool Platformer2App::IsSolidTile(int x, int y) const
{
	...
	return level[static_cast<SizeT>(y)][static_cast<SizeT>(x)].kind == TileKind::Solid;
}
...
bool Platformer2App::IsKindInRect(const RectF& rect, TileKind kind) const
{
	...
	for (int y = top; y <= bottom; ++y)
	{
		for (int x = left; x <= right; ++x)
		{
			if (level[static_cast<SizeT>(y)][static_cast<SizeT>(x)].kind == kind)
			{
				return true;
			}
		}
...
void Platformer2App::DrawEditorWindows()
{
	...
	{
		for (SizeT i = 0; i < enemies.size(); ++i)
		{
	...
	{
		TileCell& cell = level[static_cast<SizeT>(editorSelection.y)][static_cast<SizeT>(editorSelection.x)];
...
const char* Platformer2App::SelectionName() const
{
	...
	{
	case EditorSelectionType::Tile:
		return "Tile";
	case EditorSelectionType::Enemy:
		return "Enemy";
	case EditorSelectionType::Lift:
		return "Lift";
	case EditorSelectionType::PlayerSpawn:
		return "Player Spawn";
	case EditorSelectionType::Goal:
		return "Goal";
	default:
		return "None";
	}
