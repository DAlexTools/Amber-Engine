...
constexpr uint32 CategoryDefault = 0x00000001u;
constexpr uint32 CategoryRed = 0x00000002u;
constexpr uint32 CategoryBlue = 0x00000004u;
constexpr uint32 CategorySensor = 0x00000008u;
...
void PhysicsLabApp::BuildContainerScene()
{
	...
	{
		const AE::Math::FVector2D position = ContainerToWorld(ContainerParticleLocalPosition(static_cast<SizeT>(index)));
...
void PhysicsLabApp::BuildCollisionFiltersScene()
{
	...
	const uint32 redMask = filterRedBlue ? (0xFFFFFFFFu & ~CategoryBlue) : 0xFFFFFFFFu;
	const uint32 blueMask = filterRedBlue ? (0xFFFFFFFFu & ~CategoryRed) : 0xFFFFFFFFu;
...
void PhysicsLabApp::RespawnLostBodies()
{
	...
	{
		for (SizeT index = 0; index < containerParticles.size(); ++index)
		{
...
AE::Math::FVector2D PhysicsLabApp::ContainerParticleLocalPosition(SizeT index) const
{
...
{
	for (SizeT index = 0; index < containerWalls.size() && index < containerWallSpecs.size(); ++index)
	{
