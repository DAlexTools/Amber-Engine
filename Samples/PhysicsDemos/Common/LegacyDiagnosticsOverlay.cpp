...
std::string Truncate(const std::string& value, std::size_t maxChars)
{
...
{
    const std::uint64_t now = Counter();
...
void LegacyDiagnosticsOverlay::Draw(const LegacyDiagnosticsData& data) const
{
    ...
    if (showPerformance)
    {
        ...
        if (hasWorld)
        {
            ...
            DrawTextLine(
                22,
                y,
                "Islands: " + std::to_string(stats.solverIslandCount)
                    + " largest " + std::to_string(stats.largestSolverIslandBodyCount)
                    + "/" + std::to_string(stats.largestSolverIslandConstraintCount));
            DrawTextLine(
                22,
                y,
                "Parallel N/S: " + std::string(stats.parallelNarrowPhaseUsed ? "on" : "off")
                    + "/" + (stats.parallelSolverUsed ? "on" : "off"));
            DrawTextLine(
                22,
                y,
                "Jobs N/S: " + std::to_string(stats.parallelNarrowPhaseJobs)
                    + "/" + std::to_string(stats.parallelSolverJobs));
        }
    }
    ...
    if (showOutputLog)
    {
        ...
        const std::size_t visibleLines = 9u;
        const std::size_t firstEntry = entries.size() > visibleLines ? entries.size() - visibleLines : 0u;
        const std::size_t maxTextChars = static_cast<std::size_t>(std::max(20, (panelWidth - 24) / 8));
...
std::uint64_t LegacyDiagnosticsOverlay::Counter()
{
    return SDL_GetPerformanceCounter();
}

double LegacyDiagnosticsOverlay::ElapsedMilliseconds(std::uint64_t startCounter)
{
    const std::uint64_t endCounter = Counter();
    const double frequency = static_cast<double>(SDL_GetPerformanceFrequency());
